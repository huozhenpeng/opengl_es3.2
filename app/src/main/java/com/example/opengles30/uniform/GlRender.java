package com.example.opengles30.uniform;
...
public class GlRender implements GLSurfaceView.Renderer {
    ...
    public void onSurfaceCreated(GL10 gl, EGLConfig config) {
        ...
        //着色器
        vShader = ShaderUtils.loadShaderFromAssets(context, GLES32.GL_VERTEX_SHADER, "triangle_uniform.vsh");
        fShader = ShaderUtils.loadShaderFromAssets(context, GLES32.GL_FRAGMENT_SHADER, "triangle_uniform.fsh");
    ...
    private void setColor() {
         ...
         */
        int vertexColorLocation = GLES32.glGetUniformLocation(shaderProgram, "outColor");
        if (vertexColorLocation == -1) {
            VLog.Loge(TAG, "没有找到outColor位置");
        } else {
            float greenValue = (float) Math.random();
            GLES32.glUniform4f(vertexColorLocation, 0.0f, greenValue, 0.0f, 1.0f);
        }
    }
